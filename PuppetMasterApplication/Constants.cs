...
namespace PuppetMasterApplication
{
    internal partial class PuppetMaster
    {
        ...
        private const String    SERVICE_NAME = "puppet",
                                ...
                                OPERATOR_ID_COMMAND = @"^\ *(\d+) INPUT_OPS ((?:(?:(?:\d+)|(?:(?:[A-Z]:(?:\\|\/))?(?:.+(?:\\|\/))*.+(?:\\|\/)?))\,)*(?:(?:\d+)|(?:(?:[A-Z]:(?:\\|\/))?(?:.+(?:\\|\/))*.+(?:\\|\/)?))) REP_FACT (\d+) ROUTING ((?:primary)|(?:hashing)|(?:random)) ADDRESS ((?:tcp:\/\/(?:(?:(?:\d{1,3}\.){3}\d{1,3})|(?:localhost)):(?:\d{1,5})\/(?:\w+)\,)*tcp:\/\/(?:(?:(?:\d{1,3}\.){3}\d{1,3})|(?:localhost)):(?:\d{1,5})\/(?:\w+)) OPERATOR_SPEC ((?:UNIQ (\d+))|(?:COUNT)|(?:DUP)|(?:FILTER (?:\d+), (?:>|<|=), (?:\d+))|(?:CUSTOM (?:\w+), (?:\w+), (?:\w+)))\ *$",
                                START_COMMAND = @"^\ *Start (\d+)\ *$",
                                INTERVAL_COMMAND = @"^\ *Interval (\d+)\ *$",
